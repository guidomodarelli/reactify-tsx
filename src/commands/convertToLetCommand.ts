...
import {
  ...
  VARIABLE_CONVERSION_TO_LET_SUCCESS_MESSAGE,
  VARIABLE_CONVERSION_UNSUPPORTED_MESSAGE,
} from '../constants/messages';

const service = new VariableKindConversionService();

export async function convertToLetCommand(): Promise<void> {
  ...
  const result = service.createConversionPlan(document, selection, 'let');
  ...
  vscode.window.showInformationMessage(VARIABLE_CONVERSION_TO_LET_SUCCESS_MESSAGE);
}
